...
def create_index(persist_directory, embedding_model):
    ...
    texts = CharacterTextSplitter.from_tiktoken_encoder(encoding_name)
...
def build_graph(model_name, memory):
    ...
    def chatbot(state: State):
        state['messages'] = character + state['messages']
        message_response = llm_with_tools.invoke(state["messages"])
        return {"messages": state['messages'] + [message_response]}
